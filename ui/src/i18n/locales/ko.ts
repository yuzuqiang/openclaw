...
export const ko: TranslationMap = {
  ...
  newSession: {
    ...
    cloudRequiresOpenClawRuntime:
      "클라우드 워커는 OpenClaw 런타임이 필요합니다. 현재 {runtime}이(가) 선택되어 있습니다.",
  ...
  sessionsView: {
    ...
    sessionState: "스레드 상태",
    viewArchived: "보관함 보기",
    sessionArchived: "스레드가 보관됨",
    sessionsArchived: "스레드 {count}개 보관됨",
    deleteAllArchived: "보관된 항목 모두 삭제…",
    deleteAllArchivedConfirm: "보관된 스레드 {count}개와 해당 전사본을 삭제할까요?",
  ...
  configView: {
    ...
    sessionObserver: {
      title: "세션 관찰자",
      hint: "소형 유틸리티 모델로 장시간 실행되는 세션을 요약합니다.",
      toggle: "세션 관찰자",
      toggleHint: "구독된 Control UI 세션에 대한 실시간 상태 요약을 생성합니다.",
      resolvedModel: "확인된 소형 모델",
      modelAuto: "자동 ({model})",
  ...
  subtitles: {
    ...
    advanced: "남은 모든 구성 섹션과 원본 파일 편집기.",
    debug: "스냅샷, 이벤트, RPC.",
    logs: "실시간 Gateway 로그.",
  ...
  custodian: {
    ...
    sensitiveReply: "민감한 응답이 전송되었습니다",
    send: "보내기",
    ...
    history: {
      ...
      loading: "최근 변경 사항 불러오는 중…",
      empty: "아직 기록된 변경 사항이 없습니다.",
      requestFailed: "OpenClaw가 변경 내역을 불러올 수 없습니다.",
      loadMore: "더 보기",
  ...
  labsPage: {
    ...
    swarm: {
      title: "Swarm",
      description: "Code Mode가 서브에이전트 그룹을 병렬로 조율할 수 있게 합니다.",
  ...
  appsPage: {
    ...
    cards: {
      ios: {
        title: "iPhone",
        desc: "iOS에서 채팅하고, 통화하고, 작업을 승인하고, OpenClaw로 공유하세요.",
      },
      android: {
        title: "Android",
  ...
  profilePage: {
    ...
    identity: {
      title: "ID",
      description: "이 gateway에서의 내 프로필입니다.",
      loading: "내 정보를 불러오는 중…",
      profileUnavailable: "내 정보 프로필을 불러올 수 없습니다.",
      notSet: "정보가 설정되지 않았습니다.",
      setIdentity: "정보 설정",
      avatar: "아바타",
      ...
      openSettings: "{name}의 정보 설정 열기",
      avatarErrors: {
        invalid: "이 이미지를 처리할 수 없습니다.",
        sourceTooLarge: "10 MB 이하의 이미지를 선택하세요.",
        tooLarge: "처리된 아바타가 512 KB보다 큽니다.",
      },
    },
  },
  tasksPage: {
  ...
  gatewayLogs: {
    ...
    autoFollow: "자동 따라가기",
    file: "파일: {file}",
    truncated: "로그 출력이 잘렸습니다. 최신 부분을 표시합니다.",
    empty: "로그 항목이 없습니다.",
  },
  pluginTabs: {
  ...
  board: {
    ...
    widget: {
      cellLabel:
        "대시보드 위젯: {title}. 화살표 키로 이동하세요. Alt를 누른 채 화살표 키를 눌러 이동합니다.",
      moveHandle: "{title} 이동",
      ...
      appLoading: "앱 복원 중…",
      appStaleTitle: "이 고정된 앱이 오래되었습니다",
      appStaleDetail: "해당 서버, 리소스 또는 원본 전사본을 더 이상 사용할 수 없습니다.",
      retry: "재시도",
      ...
      sandboxOriginRequired:
        "반복적인 새로 고침 시도 후에도 위젯 인증에 실패했습니다. gateway가 위젯 샌드박스 포트를 라우팅하지 않는 리버스 프록시나 터널 뒤에서 실행되는 경우, mcp.apps.sandboxOrigin을 샌드박스 리스너로 라우팅되는 전용 공개 origin으로 설정하세요.",
  ...
  workboard: {
    widget: {
      ...
      openBoard: "보드 열기",
      statusCounts: "상태별 카드",
      noActiveCards: "준비되었거나 실행 중인 카드가 없습니다.",
    },
  ...
  chat: {
    ...
    sessionHeader: {
      ...
      copied: "복사됨",
      branches: "스레드 브랜치",
      branchSwitchUnavailable: "에이전트가 작업 중일 때는 브랜치를 전환할 수 없습니다.",
      branchSwitchRequiresAdmin: "브랜치 전환에는 운영자 관리자 권한이 필요합니다.",
      untitledBranch: "제목 없는 브랜치",
      oneMessage: "{count}개의 메시지",
    ...
    board: {
      ...
      dockMenu: "채팅 독: {dock}",
      dockLeft: "채팅을 왼쪽에 고정",
      dockRight: "채팅을 오른쪽에 고정",
      dockBottom: "채팅을 아래쪽에 고정",
      dockHidden: "채팅 숨기기",
      resizeDock: "채팅 독 크기 조정",
      reopenChat: "채팅 표시",
      workboardCard: "워크보드 카드: {title}, {status}",
      defaultTab: "메인",
      mockPlaceholder: "보드 뷰 이음새 · 탭 {tabs}개 · 위젯 {widgets}개",
      mockOverview: "개요",
      mockResearch: "리서치",
    ...
    workspaceConflict: {
      ...
      description:
        "OpenClaw가 로컬 버전을 유지하고 다른 클라우드 변경 사항을 적용했습니다. 스테이징된 결과를 검사하거나 충돌한 경로에 대해 클라우드 버전을 선택하세요.",
      ...
      copyTakeCommand: "클라우드 선택 명령 복사",
      commandHelp:
        "Bash 또는 zsh(Windows에서는 Git Bash)에서 실행하세요. inspect가 경로가 존재하지 않는다고 하면 클라우드가 삭제한 것이므로, 확인 후 로컬 경로를 수동으로 제거하세요. checkout이 파일/디렉터리 충돌을 보고하면, 방해가 되는 로컬 경로를 이동하거나 제거한 다음 다시 시도하세요. 스테이징된 ref가 없으면 알림이 오래된 것이므로 로컬 경로를 변경하지 마세요.",
      commandsUnavailable:
        "이 파일 이름에는 터미널 제어 문자가 포함되어 있어 OpenClaw가 복사 가능한 셸 명령을 생성하지 않습니다. 스테이징된 ref를 직접 검사하고 경로를 신중하게 수동으로 입력하세요.",
      ...
      eventDescription:
        "이 경로들에는 로컬 버전이 유지되었으며, 다른 클라우드 변경 사항이 적용되었습니다.",
    },
    compaction: {
      label: "압축된 기록",
    ...
    observer: {
      ...
      askBusy: "옵서버가 이미 질문에 답변하고 있습니다.",
      askUnavailable: "옵서버가 지금은 답변할 수 없습니다.",
      health: {
        "on-track": "정상 진행",
        grinding: "작업 중",
        stuck: "막힘",
    ...
    composer: {
      ...
      holdToRecordSettingDescription:
        "작성기의 마이크 버튼을 길게 누르고 말한 다음, 놓으면 전송하지 않고 텍스트가 삽입됩니다.",
      ...
      dictationProviderUnavailable: "받아쓰기에 구성된 전사 공급자가 없습니다.",
  ...
  cron: {
    ...
    form: {
      ...
      script: "스크립트",
      readOnlyPayloadHelp:
        "이 페이로드는 Control UI 외부에서 생성되었습니다. 그 내용은 읽기 전용으로 유지되며 다른 변경 사항을 저장할 때 보존됩니다.",
