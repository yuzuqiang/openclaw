...
export const ja_JP: TranslationMap = {
  ...
  sessionsView: {
    ...
    cloudWorkerPlacementConflict: "クラウドワーカー: {state} · ワークスペースの競合 1 件",
    cloudWorkerPlacementConflicts: "クラウドワーカー: {state} · ワークスペースの競合 {count} 件",
    cloudWorkerDescendantConflict: "クラウドワーカーの子: ワークスペースの競合 1 件",
  ...
  gatewayLogs: {
    ...
    autoFollow: "自動追従",
    file: "ファイル: {file}",
    truncated: "ログ出力が切り詰められています。最新の部分を表示しています。",
    empty: "ログエントリはありません。",
  },
  pluginTabs: {
  ...
  chat: {
    ...
    workspaceConflict: {
      titleOne: "クラウドワークスペースの競合が1件あります",
      titleMany: "クラウドワークスペースの競合が{count}件あります",
      description:
        "OpenClaw はローカルバージョンを保持し、競合していないクラウドの変更を適用しました。ステージングされた結果を確認するか、競合しているパスにクラウド側のバージョンを適用してください。",
      morePaths: "ほか{count}件のパス",
      stagedResult: "ステージングされたクラウドの結果",
      copyStagedResult: "ステージングされた結果の参照をコピー",
      inspectCloud: "最初のクラウドバージョンを確認",
      takeCloud: "最初のクラウドバージョンを適用",
      copyInspectCommand: "クラウド確認コマンドをコピー",
      copyTakeCommand: "take-cloud コマンドをコピー",
      commandHelp:
        "これらを Bash または zsh（Windows では Git Bash）で実行してください。確認時にパスが存在しないと表示された場合、クラウド側で削除されています。確認のうえ、ローカルパスを手動で削除してください。checkout でファイルとディレクトリの競合が報告された場合、妨げとなっているローカルパスを移動または削除してから、再試行してください。ステージングされた参照が見つからない場合、この通知は古くなっています。ローカルパスを変更しないでください。",
      commandsUnavailable:
        "このファイル名にはターミナル制御文字が含まれているため、OpenClaw はコピー可能なシェルコマンドを生成しません。ステージングされた参照を直接確認し、注意してパスを手動入力してください。",
      dismiss: "ワークスペースの競合通知を閉じる",
      eventSender: "クラウドワークスペース",
      eventTitleOne: "クラウドの結果が適用され、1件の競合が発生しました",
      eventTitleMany: "クラウドの結果が適用され、{count}件の競合が発生しました",
      eventDescription:
        "これらのパスではローカルバージョンが保持され、その他のクラウドの変更が適用されました。",
    },
    compaction: {
      label: "圧縮された履歴",
