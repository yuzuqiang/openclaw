...
export const ja_JP: TranslationMap = {
  ...
  channels: {
    ...
    whatsapp: {
      ...
      loggedOut: "ログアウトしました。",
      logoutNotCleared:
        "保存されたWhatsAppセッションはクリアされませんでした。すでに存在しないか、認証ディレクトリを手動でクリーンアップする必要がある可能性があります。",
    },
  ...
  newSession: {
    ...
    gitCheckUnavailable:
      "このフォルダのGitを確認できませんでした。再度選択してやり直してください。",
  ...
  sessionsView: {
    ...
    deleteAllArchivedConfirm:
      "アーカイブされた{count}件のスレッドとそのトランスクリプトを削除しますか？",
    ...
    cloudWorkerPlacementConflict: "クラウドワーカー: {state} · 1件のワークスペース競合",
    cloudWorkerPlacementConflicts: "クラウドワーカー: {state} · {count}件のワークスペース競合",
    cloudWorkerDescendantConflict: "クラウドワーカーの子: 1件のワークスペース競合",
  ...
  configView: {
    ...
    sessionObserver: {
      title: "セッションオブザーバー",
      hint: "長時間実行されるセッションを小さなユーティリティモデルで要約します。",
      toggle: "セッションオブザーバー",
      toggleHint: "購読中の Control UI セッションのライブステータスダイジェストを生成します。",
      resolvedModel: "解決済みの小型モデル",
      modelAuto: "自動 ({model})",
  ...
  custodian: {
    ...
    history: {
      ...
      opaqueChange: "表示可能な構成パスの変更なしに、書式設定またはコメントが変更されました。",
      sources: {
  ...
  mcpServers: {
    ...
    connectRequired: "MCPサーバーを変更するには Gateway に接続してください。",
  ...
  labsPage: {
    intro:
      "Labs には、リリース間で変更、破損、または消滅する可能性がある実験的な機能が含まれています。",
    sectionTitle: "実験的な機能",
    ...
    codeMode: {
      title: "Code Mode",
      description:
        "エージェントがツールをコンパクトでサンドボックス化された JavaScript ワークフローに組み合わせられるようにします。",
    },
    swarm: {
      title: "Swarm",
      description: "Code Modeで複数のサブエージェントを並列にオーケストレーションできます。",
  ...
  appsPage: {
    ...
    cards: {
      ...
      android: {
        title: "Android",
        desc: "Androidスマートフォンを完全なOpenClawノードに — チャット、カメラ、Canvas。",
      },
      appleWatch: {
        title: "Apple Watch",
        desc: "手首から一目でわかるチャットとクイック返信。",
      },
      wearOs: {
        title: "Wear OS",
        desc: "AndroidコンパニオンがOpenClawを時計まで拡張します。",
      },
      macos: {
        title: "macOS",
      ...
      chrome: {
        title: "Chrome拡張機能",
        desc: "OpenClawに既存のChromeを操作させます — タブ、ページ、フォーム。",
      },
      plugins: {
        title: "プラグインとClawHub",
        desc: "コミュニティのチャネル、ツール、スキルでOpenClawを拡張します。",
      },
    },
  },
  presence: {
  ...
  profilePage: {
    ...
    identity: {
      title: "アイデンティティ",
      description: "この Gateway でのプロフィールです。",
  ...
  gatewayLogs: {
    title: "ログ",
    subtitle: "Gateway のファイルログ (JSONL)。",
  ...
  board: {
    ...
    announcement: {
      ...
      granted: "ウィジェットアクセスを許可しました。",
      rejected: "ウィジェットアクセスを拒否しました。",
    },
    widget: {
      cellLabel:
        "ダッシュボードウィジェット: {title}。矢印キーで移動できます。Alt を押しながら矢印キーで位置を変更します。",
      ...
      sandboxOriginRequired:
        "更新を繰り返し試みましたが、ウィジェットの認証に失敗しました。ウィジェットのサンドボックスポートをルーティングしないリバースプロキシまたはトンネルの背後で Gateway を実行している場合は、mcp.apps.sandboxOrigin にサンドボックスリスナーへルーティングされた専用のパブリックオリジンを設定してください。",
      errorTitle: "このウィジェットを読み込めませんでした",
      errorDetail: "この問題はこのカードに限定されています。",
  ...
  workboard: {
    widget: {
      ...
      cardIdRequired: "このウィジェットには cardId プロップが必要です。",
  ...
  chat: {
    ...
    sessionHeader: {
      ...
      copied: "コピーしました",
      branches: "スレッドのブランチ",
      branchSwitchUnavailable: "エージェントの作業中はブランチの切り替えができません。",
    ...
    workspaceConflict: {
      ...
      description:
        "OpenClaw はローカル版を保持し、他のクラウド変更を適用しました。ステージされた結果を確認するか、競合したパスにそのクラウド版を採用してください。",
      morePaths: "他{count}件のパス",
      stagedResult: "ステージされたクラウド結果",
      copyStagedResult: "ステージ結果の参照をコピー",
      inspectCloud: "最初のクラウド版を確認",
      takeCloud: "最初のクラウド版を採用",
      copyInspectCommand: "クラウド確認コマンドをコピー",
      copyTakeCommand: "take-cloud コマンドをコピー",
      commandHelp:
        "これらは Bash または zsh（Windows では Git Bash）で実行してください。inspect でパスが存在しないと表示された場合、クラウドがそれを削除しています。確認のうえローカルパスを手動で削除してください。checkout がファイル/ディレクトリの競合を報告した場合、ブロックしているローカルパスを移動または削除してから再試行してください。ステージされた参照が見つからない場合、その通知は古いものです。ローカルパスは変更しないでください。",
      commandsUnavailable:
        "このファイル名には端末制御文字が含まれているため、OpenClaw はコピー可能なシェルコマンドを作成しません。ステージ参照を直接確認し、パスは注意して手動で入力してください。",
      dismiss: "ワークスペース競合通知を閉じる",
      eventSender: "クラウドワークスペース",
      eventTitleOne: "クラウド結果を適用（競合1件）",
      eventTitleMany: "クラウド結果を適用（競合{count}件）",
      eventDescription: "これらのパスはローカル版が保持され、他のクラウド変更が適用されました。",
    },
    compaction: {
      label: "圧縮された履歴",
    ...
    messages: {
      ...
      hideTooltip: "このブラウザでのみ非表示にする",
      openInCanvas: "キャンバスで開く",
      reply: "返信",
      replyToMessage: "メッセージに返信",
      rewind: "巻き戻し",
      rewindConfirm: "このメッセージより前に巻き戻しますか？",
    ...
    observer: {
      ...
      askPlaceholder: "なぜそうしているの？",
      askSubmit: "確認",
      askPending: "観察内容を確認しています…",
    ...
    composer: {
      ...
      cameraPageInactive: "このページが非アクティブの間はカメラを使用できません。",
      ...
      holdToRecordSettingDescription:
        "コンポーザーのマイクボタンを押し続けて話し、離すと送信せずにテキストが挿入されます。",
      dictationAudioUnsupported:
        "Gatewayがサポートされていない音声入力オーディオ形式を返しました。",
      dictationBrowserAudioUnsupported: "このブラウザは8 kHzでの音声入力を取得できません。",
      ...
      dictationFinalizationTimedOut:
        "最後の途中経過の文字起こしが確定される前に音声入力が停止しました。",
      ...
      selectedCameraUnavailable:
        "選択したカメラは利用できません。別のカメラまたはSystem defaultを選択してください。",
  ...
  cron: {
    ...
    form: {
      ...
      script: "スクリプト",
      readOnlyPayloadHelp:
        "このペイロードはControl UIの外部で作成されました。内容は読み取り専用のままで、他の変更を保存しても保持されます。",
