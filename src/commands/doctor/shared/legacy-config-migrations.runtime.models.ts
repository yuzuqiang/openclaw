  ...
  for (const [providerId, providerValue] of Object.entries(providers)) {
    ...
    for (const [modelIndex, modelValue] of models.entries()) {
      ...
      const catalogRow = catalogRows
        .get(normalizedCatalogModelKey(providerId, modelId))
        ?.find((row) =>
          modelTransportRoutesMatch(row, {
            api: configuredRoute.api ?? row.api,
            baseUrl: configuredRoute.baseUrl ?? row.baseUrl,
          }),
        );
